...
async def upsert_documents(
    ...
    raw_ids = [doc[0] for doc in documents]
    ...
    # Convert every raw string ID into a deterministic UUID v5 so the same

    # source document always maps to the same point (idempotent upserts).
    ...
    def _to_uuid(raw: str) -> str:
        ...
        try:
            ...
            uuid.UUID(raw)  # already a valid UUID — use as-is
    ...
    ids = [_to_uuid(rid) for rid in raw_ids]
    ...
    points = [
        ...
        PointStruct(
            ...
            # Keep the original string ID in the payload for traceability

            payload={"text": text, "doc_id": raw_id, **metadata},

        )

        for point_id, raw_id, vector, text, metadata in zip(

            ids, raw_ids, vectors, texts, metadatas
